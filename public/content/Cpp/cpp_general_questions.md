...
Ключевое слово `extern` в C++ используется для объявления переменной или функции без её определения, указывая, 
что определение находится в другом файле или модуле.

**Основные случаи использования:**
1. Объявление глобальных переменных, определённых в другом файле

```c++
// В file1.cpp переменная определяется:
int globalVar = 42;  // определение

// В file2.cpp используется extern, чтобы получить доступ к той же переменной:
extern int globalVar;  // объявление (без создания новой переменной)
```
2. Объявление функций, определённых в другом файле
```c++
void foo() { /* ... */ }  // определение
extern void foo();  // объявление (можно без extern, но иногда используется для явности)
```
3. Указание внешнего связывания (extern "C") для совместимости с C
```c++
extern "C" {
    void c_function();  // будет доступна под именем "c_function" без C++-декораций
}
```
**Важно:**
- `extern` не выделяет память, а лишь указывает компилятору, что переменная/функция определена где-то ещё.
- Для константных переменных (`const`) в C++ нужно явно указывать `extern`, иначе они имеют внутреннее связывание (как static).
